...
public class StevenHallAudio : MonoBehaviour {

    public GameObject Steven;
    CircleCollider2D[] StevenColliders;
    ...
    void Start()
    {
        StevenColliders = Steven.GetComponents<CircleCollider2D>();
    ...
    void Update()
    {
        ...
        if (PlayerColliders[1].IsTouching(StevenColliders[0]) && NPCSubtitlePanel.activeSelf)
        {
            if (IsPlaying == false)
            {
                IsPlaying = true;
    ...
    void StevenAudioPlay()
    {
        switch (AudioSwitch)
        {
            case 0:
                AudioSwitch++;
                AudioToPlay("PamelaCustom_01", 0.5f);
                break;
            case 1:
                AudioLength();
                AudioSwitch++;
                AudioToPlay("PamelaCustom_02", 0.5f);
                break;
            case 2:
                AudioLength();
                AudioSwitch++;
                AudioToPlay("StevenCustom_02", 1.5f);
                break;
            case 3:
                AudioLength();
                AudioSwitch++;
                AudioToPlay("PamelaCustom_03", 1.0f);
                break;
            case 4:
                AudioLength();
                AudioSwitch++;
                AudioToPlay("StevenCustom_01", 2.0f);
                break;
            case 5:
                AudioLength();
                AudioSwitch++;
